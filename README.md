...
컨트롤 바의 **반자동** 체크박스를 선택하면 반자동 모드로 전환됩니다.

- 프롬프트가 각 패널의 입력란에 자동으로 채워집니다.
- 해당 패널에 **주황색 깜빡임 하이라이트**가 표시됩니다.
- 사용자가 내용을 확인한 뒤 **직접 전송 버튼을 클릭**합니다.
- 전송 후 응답 캡처는 자동으로 진행됩니다.

반자동 모드는 사용자가 전송할 때까지 무제한 대기하므로, 각 메시지를 확인하고 필요 시 수정한 뒤 보낼 수 있습니다.
...
TalkBoard는 각 AI 서비스의 **실제 웹 페이지**를 Electron WebContentsView로 로드하고, DOM 조작을 통해 프롬프트를 입력하고 응답을 캡처합니다. 이는 브라우저 자동화(Selenium, Puppeteer 등)와 동일한 방식입니다.

### 각 서비스 이용약관(ToS) 관련

| 서비스 | 약관 요약 | 위험 수준 |
|--------|----------|----------|
| **OpenAI (ChatGPT)** | 자동화된 수단으로 서비스에 접근하는 것을 금지 | 중 |
| **Google (Gemini)** | 자동화된 수단 사용 금지 | 중 |
| **Anthropic (Claude)** | 유사한 자동화 접근 제한 조항 존재 | 중 |

### 권장 사항

- **반자동 모드 사용 권장**: 텍스트 입력만 자동화하고 전송은 사용자가 직접 수행하므로, 자동화 수준이 낮아 감지 위험이 줄어듭니다.
- **과도한 사용 자제**: 짧은 시간에 대량의 요청을 보내면 rate limit에 걸리거나 계정이 제한될 수 있습니다.
- **개인 용도로 사용**: 상업적 대규모 자동화가 아닌, 개인 학습·비교·연구 목적으로 사용하세요.
- **공식 API 대안**: 약관 준수가 중요한 경우, 각 서비스의 공식 API를 사용하는 것이 가장 안전합니다. 다만 API 사용에는 별도 비용이 발생합니다.

> **면책 조항**: TalkBoard는 오픈소스 도구이며, 사용에 따른 책임은 사용자에게 있습니다. 각 AI 서비스의 이용약관을 확인하고 준수하시기 바랍니다.
